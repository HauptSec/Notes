...
wget https://download.splunk.com/products/splunk/releases/"${v}"/linux/splunk-"${v}"-"${b}"-Linux-x86_64.tgz
tar -C /opt -xzf splunk-"${v}"-"${b}"-Linux-x86_64.tgz
ln -sf /opt/splunk/bin/splunk /usr/bin/splunk
